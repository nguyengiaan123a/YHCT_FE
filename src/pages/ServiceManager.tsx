...
const ServiceManager = () => {
  ...
  // Khởi tạo form data với các trường mới của ServicesproVM
  const [formData, setFormData] = useState({ 
    title: '', 
    status: 1, 
    thumnail: '', 
    file: null,
    id_Catogeryservice: '', // ID Danh mục
    price: '', // Giá
    description: '' // Mô tả
  });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const sortedData = rawData.sort((a, b) => new Date(b.createdDate) - new Date(a.createdDate));
  ...
  const fetchCategories = async () => {
    try {
      // Gọi API với pageSize lớn để lấy hết danh mục cho dropdown
  ...
  useEffect(() => {
    fetchCategories(); // Gọi 1 lần khi load trang
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      submitData.append('id_Catogeryservice', formData.id_Catogeryservice.toString());
      ...
      if (editingId) {
        await apiClient.put('/api/cap-nhat-dich-vu', submitData, {
          params: { id: editingId },
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      } else {
        await apiClient.post('/api/them-dich-vu', submitData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      await apiClient.delete(`/api/xoa-dich-vu`, { params: { id: itemToDelete } });
  ...
  const openEdit = (item) => {
    ...
    setFormData({ 
      ...
      file: null,
      id_Catogeryservice: item.id_Catogeryservice || '',
      price: item.price || '',
  ...
  const resetForm = () => {
    setEditingId(null);
    setFormData({ title: '', status: 1, thumnail: '', file: null, id_Catogeryservice: '', price: '', description: '' });
  };
  ...
  const getCategoryName = (id) => {
    ...
    return cat ? cat.title : 'Chưa xác định';
  };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] w-full overflow-hidden bg-white font-sans text-gray-800">
      ...
      <div className="px-5 py-3 border-b border-gray-200 flex flex-col md:flex-row justify-between items-center gap-3 bg-white shrink-0">
        <div>
          <h1 className="text-xl font-bold tracking-tight">Quản lý Dịch vụ</h1>
          <p className="text-xs text-gray-500">Danh sách các dịch vụ đang cung cấp</p>
        </div>
        ...
        <table className="w-full text-left border-collapse min-w-[900px]">
          <thead className="sticky top-0 z-10 bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="px-5 py-3 font-bold text-xs uppercase text-gray-600">Tên dịch vụ</th>
              <th className="px-5 py-3 font-bold text-xs uppercase text-gray-600">Danh mục</th>
              <th className="px-5 py-3 font-bold text-xs uppercase text-gray-600 text-right">Giá tiền</th>
            ...
            {loading ? (
              ...
              services.map((item, index) => (
                <tr key={item.id} className="hover:bg-gray-50/80 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <div className="font-semibold text-sm text-blue-700">{item.title}</div>
                    <div className="text-[10px] text-gray-400 uppercase tracking-tighter">ID: {item.id} | {formatDate(item.createdDate)}</div>
                  </td>
                  <td className="px-5 py-3">
                    <span className="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded-md border border-gray-200">
                      {getCategoryName(item.id_Catogeryservice)}
                    </span>
                  </td>
                  <td className="px-5 py-3 text-right font-semibold text-orange-600 text-sm">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-[999]">
          {/* Tăng max-width và set max-height để cuộn form nếu nội dung dài */}
          <div className="bg-white rounded-xl w-full max-w-lg shadow-xl overflow-hidden border border-gray-200 flex flex-col max-h-[90vh]">
            <div className="px-5 py-3 border-b border-gray-100 flex justify-between items-center bg-gray-50 shrink-0">
              <h3 className="text-sm font-bold uppercase text-gray-700">{editingId ? 'Chỉnh sửa Dịch vụ' : 'Thêm Dịch vụ mới'}</h3>
            ...
            <div className="p-5 overflow-y-auto flex-1">
              <form id="serviceForm" onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-[11px] font-bold text-gray-500 uppercase mb-1">Tên dịch vụ <span className="text-red-500">*</span></label>
                  <input required type="text" className="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" 
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 uppercase mb-1">Danh mục dịch vụ <span className="text-red-500">*</span></label>
                    <select required className="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" 
                      value={formData.id_Catogeryservice} onChange={(e) => setFormData({...formData, id_Catogeryservice: parseInt(e.target.value)})}
                    >
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 uppercase mb-1">Giá dịch vụ (VNĐ) <span className="text-red-500">*</span></label>
                    <input required type="number" min="0" className="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" 
                ...
                <div>
                  <label className="block text-[11px] font-bold text-gray-500 uppercase mb-1">Mô tả ngắn <span className="text-red-500">*</span></label>
                  <textarea required rows="3" className="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none" 
                  ...
                  <div className="flex gap-3 items-center">
                    ...
                    {formData.thumnail && (
                      <div className="w-12 h-12 shrink-0 bg-white border border-gray-200 rounded-lg overflow-hidden">
                        <img src={formData.file ? formData.thumnail : `${PATH_IMAGES}${formData.thumnail}`} alt="preview" className="w-full h-full object-cover" />
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <select className="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none font-medium" 
                    value={formData.status} onChange={(e) => setFormData({...formData, status: parseInt(e.target.value)})}
                  >
            ...
            <div className="p-4 border-t border-gray-100 bg-gray-50 shrink-0">
              <button form="serviceForm" type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 rounded-lg shadow-md transition-all active:scale-[0.98] text-sm uppercase">
                Lưu dữ liệu
              </button>
            </div>
